...
class TileController:
    ...
    def __next__(self):
        ...
        while True:
            ...
            while count < self.batch_size:
                ...
                if self.index >= len(self.pixel_locations):
                    self.index = 0
                # Get row, col, and dataset index from pixel locations
                row, col = self.pixel_locations[self.index][0]
                dataset_index = self.pixel_locations[self.index][1]
                self.index += 1
                # Read tile using image_datasets and specified bands and window
                tile = self.image_datasets[dataset_index].read(
                    list(np.arange(1, self.band_count + 1)),
                    window=Window(
                        col - self.buffer,
                        row - self.buffer,
                        self.tile_width,
                ...
                if self.is_valid_tile(tile):
                    ...
                    if label != 0 and not np.isnan(label):
                        label_batch[count][label] = 1
                        image_batch[count] = reshaped_tile
                        count += 1
    ...
    def is_valid_tile(self, tile):
        ...
        if np.isnan(tile).any() or -9999 in tile or 255 in tile:
            return False

        # Check if the shape of the tile matches the expected shape
        if tile.shape != (self.band_count, self.tile_width, self.tile_height):
            return False
